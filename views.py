...
import csv, io, json
from .models import SDKData, SW_Details, Target, RunDetails, Model, Config, ECC, Results, Performance
...
def upload_csv(request):
    ...
    def parse_json(field, row, default="{}"):
        try:
            return json.loads(row.get(field, default))
        except Exception:
            return {}
    ...
    for row in reader:
        try:
            ...
            model_qs = Model.objects.filter(
                ...
                model_fw=row.get("model_fw", "default_fw")
            )
            if model_qs.exists():
            ...
            result = Results(
                execution_time={
                    "compilation": row.get("compilation_time", ""),
                    "execution": row.get("execution_time", "")
                },
                cpu_stats="NA",
                device_stats={
                    "median_dram_util": row.get("median_dram_util", ""),
                    "median_dram_bw": row.get("median_dram_bw", ""),
                    "load_time": row.get("load_time", "")
                },
                model_stats=parse_json("model_stats", row),
            ...
            performance = Performance(
                result_id=result,
                performance_metrics={
                    "e2e_processed_tok_sec": row.get("e2e_processed_tok_sec", ""),
                    "e2e_generated_tok_sec": row.get("e2e_generated_tok_sec", ""),
                    "output_token_throughput(tok/s)": row.get("output_token_throughput(tok/s)", ""),
                    "mean_tttft_sec": row.get("mean_tttft_sec", ""),
                    "request_throughput(req/s)": row.get("request_throughput(req/s)", ""),
                    "total_token_throughput_tok_sec": row.get("total_token_throughput_tok_sec", "")
                }
            )
            performance.save()
    ...
    return JsonResponse({"message": f"Upload successful. Rows created: {created_count}"}, status=201)
